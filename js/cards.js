...
//  - 외교(조약) 시스템은 이 기획에서 완전히 제외한다. 조약 파기 등 외교에 종속되어 있던 종족
//    정체성은 RACE_SPECIAL_ABILITY(아래)로 분리해, 외교와 무관하게 항상 적용되는 종족 특수
//    능력으로 재배치했다 — 실제 효과 구현은 rules.js의 종족 특성 훅을 참조.
...
// 종족 특수 능력 — 카드로 쓰는 게 아니라 항상 켜져 있는 종족 고유 패시브다. 원래 기획에서
// 외교(조약) 시스템에 묶여 있던 종족별 정체성을 그쪽에서 완전히 분리해 여기로 옮겼다.
// 실제 수치 적용은 rules.js에 있고(startTurn/powerMultiplier/resolveCombat/tickInfestation),
// 여기 텍스트는 UI(종족 선택 화면, 자원 바)에 표시하는 설명용 메타데이터다.
export const RACE_SPECIAL_ABILITY = {
  kingdom: {
    name: '영지 총동원령',
    desc: '소유한 영지(territory) 노드 1개당 매 라운드 물자 +1. 넓게 펼수록 강해지는 왕국의 물량 정체성.',
  },
  cultists: {
    name: '출혈의 계약',
    desc: '전투가 벌어지면 승패와 무관하게, 양측이 잃은 전력의 15%만큼 광신을 자동으로 얻는다. 싸움 자체가 곧 자원이다.',
  },
  rift: {
    name: '차원 공명',
    desc: '차원문 티어가 오를수록 소환수뿐 아니라 모든 차원 괴물 부대의 전투력 자체가 강해진다(티어당 +8%). 게임 후반 갈수록 압도적으로 강해지는 캐리형 특성.',
  },
  infested: {
    name: '자가 증식',
    desc: '감염을 완료할 때마다 40% 확률로 인접한 노드에 대가 없이 새 감염이 자연 발생한다(감염력이 높을수록 더 빨리 완료). 포자 지대는 스스로 번져나간다.',
  },
};
